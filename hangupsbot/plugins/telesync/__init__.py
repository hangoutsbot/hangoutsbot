...
def groupIDChange(oldID, newID, bot):
    tg2ho_dict = bot.memory.get_by_path(['telesync_tg2ho'])
    ho2tg_dict = bot.memory.get_by_path(['telesync_ho2tg'])

    hangout = tg2ho_dict[str(oldID)]
    del tg2ho_dict[str(oldID)]
    tg2ho_dict[str(newID)] = str(hangout)
    ho2tg_dict[str(hangout)] = str(newID)
...
def _on_hangouts_message(bot, event, command=""):
    ...
    if event.conv_id in ho2tg_dict:
        ...
        except telepot.TelegramError as error:
            newID = error.parameters
            if newID['migrate_to_chat_id']:
                logger.info('chat id changed from %s to %s', ho2tg_dict[event.conv_id], newID)
                groupIDChange(ho2tg_dict[event.conv_id], newID['migrate_to_chat_id'], bot)
                yield from tg_bot.sendMessage(ho2tg_dict[event.conv_id], text, parse_mode='Markdown',
                                              disable_web_page_preview=True)
